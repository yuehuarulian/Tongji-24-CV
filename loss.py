...
def batch_hard(dists, pids, margin, batch_precision_at_k=None):
    ...
    with tf.name_scope("batch_hard"):
        same_identity_mask = tf.equal(tf.expand_dims(pids, axis=1), # 指示批次中每对样本是否具有相同的身份
                                      tf.expand_dims(pids, axis=0))
        negative_mask = tf.logical_not(same_identity_mask) # 指示批次中每对样本是否具有不同的身份
        positive_mask = tf.logical_xor(same_identity_mask, # 指示批次中每对样本是否为不同身份的正样本
                                       tf.eye(tf.shape(pids)[0], dtype=tf.bool))

        furthest_positive = tf.reduce_max(dists*tf.cast(positive_mask, tf.float32), axis=1) # 计算每个样本与同身份样本中距离最远的距离
        closest_negative = tf.map_fn(lambda x: tf.reduce_min(tf.boolean_mask(x[0], x[1])), # 每个样本，找到与其不同身份的样本中距离最近的距离
    ...
    with tf.name_scope("monitoring"):
        ...
        # drop the diagonal.
        _, indices = tf.nn.top_k(-dists, k=batch_precision_at_k+1) # 找到每个样本的top-k最近邻样本的索引

        # Drop the diagonal (distance to self is always least).
        indices = indices[:,1:] # 去除最近邻样本中的自身样本

        # Generate the index indexing into the batch dimension.
        # This is simething like [[0,0,0],[1,1,1],...,[B,B,B]]
        batch_index = tf.tile( # 为每个样本创建批次索引，用于检索身份标签
        ...
        topk_is_same = tf.gather_nd(same_identity_mask, topk_indices) # 根据top-k索引从相同身份掩码中检索相应的布尔值，表示最近邻样本是否具有相同的身份
        ...
        top1 = tf.reduce_mean(topk_is_same_f32[:,0]) # 计算top-1准确率，即每个样本的最近邻样本中的第一个是否具有相同的身份
        prec_at_k = tf.reduce_mean(topk_is_same_f32) # 计算top-k准确率，即每个样本的最近邻样本中的前k个是否具有相同的身份

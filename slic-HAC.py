...
from HAC import AgglomerativeClustering
import colorsys
from PIL import Image
import os
imagefile = 'data/images/'
resultfile = 'data/slic_HAC_HSV/'
gtfile = 'data/gt/'
...
class SLICProcessor(object):  # 定义SLIC处理器类
    ...
    def open_image(path):  # 静态方法，用于打开图像
        ...
        import cv2
        rgb = cv2.GaussianBlur(rgb, (5, 5), 2)
        # cv2.imshow("GaussianBlur", rgb)
    ...
    def generate_result(self, K, img_d):
        ...
        clustering.fit(temp_img)

        for i in range(len(self.clusters)):
            self.clusters[i].label = clustering.labels_[i]
...
def make_transform_matrix(image_arr, d0, ftype='high'):
    '''
    构建高斯高/低通滤波
    INPUT  -> 图像数组, 通带半径, 类型
    '''
...
if __name__ == '__main__':
    ...
    IOU = []
    for i in range(1,17):
        img_arr = np.array(Image.open(imagefile+f'/{i}.jpg').convert('L'))
        # 将图像从空间域转换到频率域
        print('将图像从空间域转换到频率域')
        f = np.fft.fft2(img_arr)
        fshift = np.fft.fftshift(f)
        F_filter1 = make_transform_matrix(img_arr, 30, 'high')# 生成高通滤波器
        result1 = fshift*F_filter1# 滤波
        img_d = np.abs(np.fft.ifft2(np.fft.ifftshift(result1)))# 将图像从频率域转换到空间域
        img_d = img_d.reshape(img_d.shape[0]*img_d.shape[1])
        img_d[img_d>4] = 1
        img_d[img_d<=4] = 0
        p = SLICProcessor(imagefile+f'/{i}.jpg', 400, 40, i)
        print('开始SLIC')
        p.iterate_10times()
        print('开始层次聚类')
        p.generate_result(2,img_d)
        waitKey()

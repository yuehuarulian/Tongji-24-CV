...
def main():
    ...
    '''训练图片的位置和标号文件market1501_train.csv'''
    ...
    # Load the data from the CSV file. pids:进程（人物标号） fids:文件标号（名）
    ...
    dataset = tf.data.Dataset.from_tensor_slices(unique_pids) # 建立数据集，每个集合都是unique_pids里面一个元素
    dataset = dataset.shuffle(len(unique_pids)) # 随机打乱

    # Constrain the dataset size to a multiple of the batch-size, so that
    # we don't get overlap at the end of each epoch.
    # 将数据集大小限制为批量大小的倍数，剩下丢弃，这样我们在每个 epoch 结尾时不会出现重叠。
    ...
    # 也就是说一个batch有batch_p个人，每个人batch_k张照片
    ...
    # prefetch() 函数的主要作用是在训练过程中异步地加载数据，使得模型的训练过程中可以与数据加载过程并行进行，从而提高了训练的效率。
    ...
    # prefix.
    '''返回预训练模型的端点（endpoints）和一个命名前缀（body_prefix）。端点是模型中的关键节点或者层，用于提取特征或者进行后续的操作'''
    endpoints, body_prefix = model.endpoints(images, is_training=True)
    '''添加一个头部层到模型中，用于将模型提取的特征映射到具体的任务空间中'''
    ...
    dists = loss.cdist(endpoints['emb'], endpoints['emb'], metric=args.metric) # 使用了损失函数模块中的 cdist 函数，用于计算嵌入向量之间的距离矩阵。
    losses, train_top1, prec_at_k, _, neg_dists, pos_dists = loss.LOSS_CHOICES[args.loss]( # 三重损失
        dists, pids, args.margin, batch_precision_at_k=args.batch_k-1)

    # Count the number of active entries, and compute the total batch loss.
    # 计算损失值的统计信息，并将这些信息记录到 TensorBoard 日志中以便于可视化和分析
    ...
    if 0 <= args.decay_start_iteration < args.train_iterations:
        learning_rate = tf.compat.v1.train.exponential_decay( # 使用指数衰减函数设置学习率
            args.learning_rate,
    ...
    with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
        train_op = optimizer.minimize(loss_mean, global_step=global_step) # 设置了一个优化器实例train_op

    # Define a saver for the complete model.
    checkpoint_saver = tf.compat.v1.train.Saver(max_to_keep=0)

    with tf.compat.v1.Session() as sess: # 创建会话
        # 恢复训练or从头开始训练
        if args.resume:
        ...
        '''模型训练'''
        with lb.Uninterrupt(sigs=[SIGINT, SIGTERM], verbose=True) as u: # 使用 lb.Uninterrupt 来捕获中断信号
            for i in range(start_step, args.train_iterations):
                ...
                if args.detailed_logs:
                    log_embs[i], log_loss[i], log_fids[i] = b_embs, b_loss, b_fids # [0,32]
